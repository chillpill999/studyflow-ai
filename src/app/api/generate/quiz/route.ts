...
export async function POST(req: Request) {
  ...
  const rateLimitKey = user ? `quiz:user:${user.id}` : `quiz:ip:${clientIp}`;
  ...
  if (!rateCheck.allowed) {
    return NextResponse.json(
      { error: `Rate limit exceeded. Please wait ${rateCheck.resetInSeconds}s before generating more quizzes.` },
      { 
        status: 429, 
